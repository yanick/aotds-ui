...
const trajectorySegmentToPath = segment => {
  switch (segment[0]) {
    case "POSITION":
      return ["M", ...coordsToSvg(segment[1])];
    case "MOVE":
      return ["l", ...coordsToSvg(segment[1])];
    default:
      return [];
  }
};
...
const Trajectory = ({ object }) => {
  let movement = object::gen_ship_movement(
    _.get(object, "orders.movement", {})
  );
...
export default ({ objects }) => (
  ...
  >
    <svg width={400} height={400} className="battlemap">
      <g>{objects.map(o => <Trajectory object={o} />)}</g>

      <g>{objects.map(o => <BattleMapObject object={o} />)}</g>
    </svg>
  </ReactSVGPanZoom>
);
